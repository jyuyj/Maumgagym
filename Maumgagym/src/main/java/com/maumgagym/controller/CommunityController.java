...
public class CommunityController {
	...
	public ModelAndView communitylist(HttpServletRequest request) {
		...
		ArrayList<BoardDTO> communityList = dao.communityList(pNum);
	...
	public ModelAndView communitysearchkeyword(HttpServletRequest request, String keyword, int category) {
		...
		ArrayList<BoardDTO> communityList = dao.communitysearchList(keywordsearch, pNum, category);
	...
	public String communitywriteok(MemberDTO to1, BoardDTO to, HttpServletRequest request, Model model)
	...
	public ModelAndView communityview(Model model, HttpServletRequest req) {
		...
		// 2. 리뷰를 가져온다.
		ArrayList<CommentDTO> commentList = cmtdao.commentList(bto);

		for (CommentDTO to : commentList) {
	...
	public String communitymodifyok(HttpServletRequest request, BoardDTO to, MemberDTO to1, Model model) {
	...
	public String communitydeleteok(HttpServletRequest request, BoardDTO to, MemberDTO to1, Model model) {
		to1.setPassword(request.getParameter("password")); // member
		to.setSeq(Integer.parseInt(request.getParameter("seq"))); // board
