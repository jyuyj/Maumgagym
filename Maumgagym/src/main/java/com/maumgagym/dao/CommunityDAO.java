...
//	
	...
	public int boardWriteOK(MemberDTO to1, BoardDTO to) throws NamingException {
	...
	public ArrayList<BoardDTO> communityList(int pageNum) {
		...
		ArrayList<BoardDTO> communityList = new ArrayList<>();
		...
		try {
			...
			// String sql = "select b.seq, c.topic, b.title, b.content, m.name ,
			// b.write_date, r.like_count, b.status from board b left join category c on(
			// b.category_seq = c.seq) left join member m on( m.seq = b.write_seq ) left
			// join reaction r on( b.seq = r.board_seq) where 9 < c.seq and c.seq < 13 order
	...
	public ArrayList<BoardDTO> communitysearchList(String keyword, int pageNum, int category) {
		...
		try {
			...
			// join reaction r on( b.seq = r.board_seq) where 9 < c.seq and c.seq < 13 order
			// by r.like_count desc";
			String sql = "select b.seq, c.topic, b.title, b.content, m.name , b.write_date, (select count(*) from likeaction l where l.board_seq = b.seq) as like_check, b.status from board b left join category c on( b.category_seq = c.seq) left join member m on( m.seq = b.write_seq )  where 9  < c.seq and c.seq < 13 and b.title like '%"
					+ keyword + "%' and c.seq like '%" + category + "%' order by b.seq desc limit ?,?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, limitNum);
			pstmt.setInt(2, board_count);
	...
	public int getSearchPageNum(String keyword, int category) { // 전체페이지 갯구 구하는 함수
		...
		try {
			...
			String sql = "select count(*) from board b left join category c on( b.category_seq = c.seq) left join member m on( m.seq = b.write_seq ) left join reaction r on( b.seq = r.board_seq) where 9  < c.seq and c.seq < 13 and b.title like '%"
					+ keyword + "%' and c.seq like '%" + category + "%' ";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public BoardDTO boardView(BoardDTO to, MemberDTO mto) {
	...
	public BoardDTO boardModify(BoardDTO to) {
	...
	public int boardModifyOK(MemberDTO to1, BoardDTO to) {
	...
	public BoardDTO boardDelete(BoardDTO to) {
	...
	public int boardDeleteOK(MemberDTO to1, BoardDTO to) {
